...
@Component({
    ...
    styleUrls: ['incoming-form.component.css']
})
...
export class IncomingFormComponent implements OnInit {
    ...
    constructor(private _categoryService: CategoryService, private _accountService: AccountService, private _incomingService: IncomingService, private router: Router, private _appService: ApplicationService, private _activatedRoute: ActivatedRoute, public dialog: MatDialog) {
        ...
        this._appService.setNavBar(this.isEditing ? 'Editar ingreso' : 'Nuevo ingreso');
    }
    ...
    _readCategories() {
        this._categoryService.getAll()
            .then((result: QuerySnapshot<Category>) => {
                ...
                this.categories = categories.filter(item => item.type === 2);
                this._readAccounts();
            })
    ...
    public save(): void {
        ...
        if (this.isEditing) {
            this._incomingService.update({ id: this.id, ...this.form.value })
                .then(() => {
                    this._appService.showToast('Ingreso modificado');
                    this.router.navigate(['/home']);
                })
                .catch(error => ErrorUtils.handleError(error));
        } else {
            this._incomingService.create(this.form.value)
                .then(() => {
                    this._appService.showToast('Ingreso registrado');
                    this.router.navigate(['/home']);
                })
                .catch(error => ErrorUtils.handleError(error));
        }
    ...
    onConfirmDelete(): void {
        ...
        dialogConfig.data = {
            text: '¿Confirma que desea eliminar el ingreso?',
            type: 'delete',
    ...
    onDelete(id: string): void {
        ...
        this._categoryService.delete(id)
            .then(() => {
                ...
                this._appService.showToast('Ingreso borrado');
                this.router.navigate(['/home'])
            })
            .catch(error => ErrorUtils.handleError(error));
    }
